...
::: {.callout-important title="Formules moeten beginnen met ="}
Een formule moet altijd beginnen met het teken \`=\`. Hoewel je een formule ook met een \`+\` kunt beginnen, zal Excel dit automatisch omzetten naar een \`=\` nadat je de gegevens hebt ingevoerd.
:::
...
[Bestand]{.smallcaps}: `Uurloon.xlsx` **Bereken werkuren**
...
![Ontwerp van een berekeningsmodel. Links 1 werkblad, rechts 3 werkbladen.](images/calculations/calculation-model.png){#fig-calculation-model}
...
::: {.callout-note title="Celbereiken"}
...
: Voorbeelden van celadressen.
:::

::: {.callout-note title="Aanpassingen celadressen bij kopieeracties"}
...
: Operatoren voor het vergelijken van twee items.
:::
...
: Voorbeelden van waar/onwaar formules.
:::

::: {.callout-important title="Gebruik dubbele aanhalingstekens voor tekst in Booleaanse formules"}
...
2.  Typ in A1 het getal `100` en in A2 het getal `275`.
...
::: {.callout-tip title="Meerdere namen selecteren"}
Om meerdere namen te selecteren moet je de [Ctrl]{.uicontrol} toets ingedrukt houden.
:::
::::

### Namen beheren {#sec-calculation-names-manager}
...
::: {.callout-note title="Bewerkingen met namen"}
...
### Namen documenteren {#sec-calculation-names-list}
...
`r hide("Hint")` 1. De dollarkoers en het BTW percentage zijn inputvariabelen voor de berekening en worden daarom gescheiden van de prijstabel in het bovenste deel van het werkblad gezet. Een toelichtende tekst staat in de cellen ervoor.
...
6.  Tot slot moeten alle eurobedragen opgemaakt worden in de financiële notatie. `r unhide()`
:::
